...
2. Open terminal and enter the Android shell by running `adb shell`
...
4. We can then verify that the Burp certificate is now listed under the system CA.
...
1. Identify the library used to implement SSL pinning
    ...
    To identify the SSL pinning library, we can do static or dynamic analysis. For static analysis, we can decompile the APK file using [jadx](https://github.com/skylot/jadx), and use the search function to search for commonly used library or related keywords such as *checkServerTrusted*, TrustManager, *CertficatePinner* or *Pinning*. 
...
Instead of manually writing our own Frida code, we can use bypass scripts shared by the community [here](https://codeshare.frida.re/). To use it, run Frida with the codeshare flag:

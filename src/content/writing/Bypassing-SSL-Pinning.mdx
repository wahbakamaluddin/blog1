...
2. **Modify the `network_security_config.xml` file**

    Add`<certificates src="user"/>` into `network_security_config.xml` so that the app trusts user CAs.
...
2. **Open terminal and enter the Android shell by running:**
    ``` bash
    adb shell
    ```
...
1. **Identify the library that is used to implement SSL pinning**
